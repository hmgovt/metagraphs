...
Stage 5 — **the bloom** (in progress). Hover a cell on the breathing field and eight coronal-flare filaments unspool from its surface along curved Bezier arcs deformed by four physical fields (emission, honesty, time, $tao), twist as the plasma cools, and settle into a steady amber afterglow in which the cell's identity / purpose / emission / signal / age / trend / network / links become legible. The detail surface lives in-canvas (filaments) + HTML overlay (text terminals), not a separate panel (D15). Hover is primary; click survives only for terminal links (D16). Snapshot schema v3 (D17) adds owner-declared description + social links, computed `daysSinceRegistration`, and four 24h/7-epoch deltas, plumbed through the same Taostats identity endpoint already used for logos. See [`docs/handoff/05-bloom.md`](docs/handoff/05-bloom.md) for the full physics + choreography + calibration brief.

Previously: Stage 4 — the breathing field. Hero-scale phyllotaxis-by-uid scatter of additively-blended bioluminescent cells filling the full viewport. Two-channel shader (intensity from `emissionShare`, temperature from `realRevenueSignal`, D11) — cold-tail subsidy-farming subnets render vivid cyan, the ~30 active subnets render saturated gold-amber, the honesty axis lands on day one. Microscope zoom (D12) with wheel + trackpad pinch + double-click. Owner-declared logos (D14, schema v2) fade in next to the cell name at zoom past `NAME_LABEL_ZOOM_THRESHOLD`. **Stage 6 lands the Yuma-epoch-locked heartbeat + births/deaths + signal refinement** — rescoped from Stage 5 so the bloom could land alone. See [`docs/handoff/`](docs/handoff/) for the staged implementation plan and [`SPEC.md`](SPEC.md) / [`DECISIONS.md`](DECISIONS.md) for the project's source of truth.
